define(["vector2", "settings", "asset_loader"], function (Vector2, Settings, AssetLoader) {
	...
	function makeNewPuck (id) {
		...
		// If it is null, then it is a ball
		// Else, check which player belongs the new puck
		if (id === -1) {
			puck.name = "ball";
			puck.size = Vector2.new(Settings.ballRadius, Settings.ballRadius);
		} else {
	...
	var proto = {
		...
		move: function (deltaTime) {
			...
			if (this.velocity.x > 0 && this.getCenterX() >= Settings.fieldWidth-this.radius-Settings.fieldPaddingX) this.velocity.x = -this.velocity.x;
			if (this.velocity.x < 0 && this.getCenterX() <= this.radius+Settings.fieldPaddingX) this.velocity.x = -this.velocity.x;
			if (this.velocity.y > 0 && this.getCenterY() >= Settings.fieldHeight-this.radius-Settings.fieldPaddingY) this.velocity.y = -this.velocity.y;
			if (this.velocity.y < 0 && this.getCenterY() <= this.radius+Settings.fieldPaddingY) this.velocity.y = -this.velocity.y;
			// Slow it down
